...
void Tutorial04_Instancing::Render()
{
    ...
    DrawAttrs.NumInstances = 10000; // The number of instances
...
void Tutorial04_Instancing::Update(double CurrTime, double ElapsedTime)
{
    ...
    float4x4 View = float4x4::RotationY(0.0f) * float4x4::RotationX(-0.5f) * float4x4::Translation(0.f, 0.0f, 30.0f);
    ...
    // **Mobile Rotation: Rotate the entire mobile, not just individual objects**
    m_RotationMatrix    = float4x4::Translation(0, -1.0f, 0);
